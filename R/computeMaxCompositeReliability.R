...
computeMaxCompositeReliability <- function(mydata, n) {
  ...
  weights = n #create a default list with the same indexes as array n
  for(i in 1:length(compRelMax$pars)) {
    weights[[names(n)[i]]] = compRelMax$pars[i] #update the value with the computed weight that maximizes composite reliability
  }

  return(computeCompositeReliability(mydata, n=n, weights=weights, FALSE))
}
